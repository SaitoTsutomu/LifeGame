...
def make_anim(obj, nx, ny, ncycle, unit):
    ...
    for tm in range(ncycle + 1):
        ...
        for y, x in product(range(ny), range(nx)):
            vtx[y, x].co.z = cells[y + 1, x + 1] * 0.5
        for v in vtx.flat:
...
class CLG_OT_make_sample(bpy.types.Operator):
    ...
    def execute(self, context):
        context.scene.nx, context.scene.ny, context.scene.ncycle = 15, 15, 8
        make_grid(context, context.scene.nx, context.scene.ny)
        ...
        bpy.ops.object.mode_set(mode="OBJECT")
        make_anim(
            obj, context.scene.nx, context.scene.ny, context.scene.ncycle, context.scene.unit
        )
        return {"FINISHED"}
...
class CLG_OT_make_anim(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not obj or obj.type != "MESH" or nx * ny != len(obj.data.vertices):
            ...
            return {"CANCELLED"}
        make_anim(obj, nx, ny, self.ncycle, self.unit)
        return {"FINISHED"}
